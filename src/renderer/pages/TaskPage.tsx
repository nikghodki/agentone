  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50">
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-200">
        <h1 className="text-xl font-semibold text-slate-900">Task Runner</h1>
      ...
      {taskStatus && (
        <div className="px-4 py-3 bg-indigo-50 border-b border-indigo-200">
          <p className="text-sm text-indigo-700">Setting up {taskStatus}...</p>
        </div>
      )}
      ...
      <div className="border-t border-slate-200 p-4">
        <div className="max-w-4xl mx-auto flex gap-3">
          <textarea
            ...
            className="flex-1 px-4 py-3 bg-white border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed resize-none"
            rows={3}
          />
          <button
            onClick={handleSendTask}
            disabled={isStreaming || !input.trim() || !currentDeploymentId}
            className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-colors"
          >
