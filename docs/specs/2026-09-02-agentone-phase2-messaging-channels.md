...
- **Slice 2c (breadth): DELIVERED (2026-09-02)** — **Slack** and **Discord** exposed on all three frameworks (openclaw, zeptoclaw, hermes) with catalog-driven config; zeptoclaw slack uses per-channel write (bot_token + app_token, no signing secret); hermes discord uses top-level `discord:` section (not `platforms.discord`); catalog supports **optional fields** (slack `signingSecret` marked optional; Connect-enabled predicate ignores optional fields); **WhatsApp-web** added as `kind:"qr"` with guided pairing flow (no credential inputs; "Start pairing" → kicks framework pairing → "Check status" verify). **DEFERRALS:** whatsapp_cloud (documented multi-field config but unverified per-channel adapter code); long-tail openclaw channels beyond the curated catalog (generic form deferred); live re-verification of zeptoclaw `channel test`/`channel list` real output and hermes per-channel connectivity (kept as documented best-effort; hermes verify remains gateway-level).
